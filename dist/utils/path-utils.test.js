...
import { getProjectRelativePath, resolveProjectDirectory, resolveProjectPath, } from "./path-utils.js";
describe("path utils", () => {
    ...
    it("rejects traversal outside project root", () => {
        expect(() => resolveProjectPath("../outside.txt", projectRoot)).toThrow(/escapes project root/);
    });
    it("rejects absolute paths outside project root", () => {
        expect(() => resolveProjectPath("/etc/passwd", projectRoot)).toThrow(/escapes project root/);
    });
    it("resolves optional directories", () => {
        expect(resolveProjectDirectory(undefined, projectRoot)).toBe(projectRoot);
        expect(resolveProjectDirectory("scenes/ui", projectRoot)).toBe(path.join(projectRoot, "scenes", "ui"));
    });
    it("computes project-relative paths", () => {
        const full = path.join(projectRoot, "resources", "data.tres");
        expect(getProjectRelativePath(full, projectRoot)).toBe(path.join("resources", "data.tres"));
    });
    it("rejects project-relative path lookup outside root", () => {
        expect(() => getProjectRelativePath("/var/tmp/file.txt", projectRoot)).toThrow(/escapes project root/);
    });
});

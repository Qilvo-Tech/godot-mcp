...
export function registerEditorTools(tools, state) {
    ...
    tools.set("godot_connection_status", {
        description: "Check the bridge connection to the Godot editor. If no socket exists in this MCP session yet, this attempts to connect before reporting — so `connected: false` means the editor/AI Bridge is actually unreachable, not merely that godot_connect hasn't been called.",
        inputSchema: z.object({}),
        handler: async () => {
            let probeError = null;
            if (!wsConnection || wsConnection.readyState !== WebSocket.OPEN) {
    ...
    tools.set("godot_editor_get_errors", {
        description: "Get structured editor/runtime errors with optional severity/query filtering and log-file inclusion.",
        inputSchema: z.object({
            includeRuntime: z
                .boolean()
                .optional()
    ...
    tools.set("godot_editor_get_output", {
        description: "Get captured output with severity/source/query filters and optional structured metadata.",
        inputSchema: z.object({
            lines: z
                .number()
                .optional()
    ...
    tools.set("godot_editor_get_log_file", {
        description: "Read and filter the latest Godot log file with structured metadata and incremental tail support.",
        inputSchema: z.object({
            lines: z
                .number()
                .optional()
        ...
        handler: async (args) => {
            await ensureConnected(state);
            const { lines = 100, filter = "all", query = "", sinceLine = 0, includeMetadata = true, } = args;
    ...
    tools.set("godot_regenerate_client_bindings", {
        description: "Regenerate SpacetimeDB GDScript client bindings via the SpacetimeDB editor addon. " +
...
function establishConnection(host, port, state) {
    if (!connectInFlight) {
...
async function ensureConnected(state) {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`Not connected to Godot editor and auto-connect failed (${message}). ` +

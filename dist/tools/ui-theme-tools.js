...
export function registerUIThemeTools(tools, state) {
    // Create a complete UI theme
    tools.set("godot_ui_create_theme", {
        description: "Create a Godot Theme resource (.tres) with colors, fonts, and styleboxes for consistent UI styling. Generates a complete theme file that can be applied to Control nodes.",
        ...
        handler: async (args) => {
            const { path: themePath, name, colors, font_size, styleboxes, preset, } = args;
            // Get preset defaults
            ...
            const themeContent = generateThemeFile(name || "UITheme", finalColors, font_size || 16, finalStyleboxes);
    ...
    tools.set("godot_ui_create_stylebox", {
        description: "Create a StyleBoxFlat resource for styling UI elements like panels, buttons, and containers.",
        inputSchema: z.object({
        ...
        handler: async (args) => {
            const { path: boxPath, bg_color, border_color, border_width, corner_radius, content_margins, shadow_color, shadow_size, } = args;
            const content = generateStyleBoxFlat(bg_color, border_color, border_width, corner_radius, content_margins, shadow_color, shadow_size);
...
// Preset styleboxes
function getPresetStyleboxes(preset) {
...
function generateThemeFile(name, colors, fontSize, styleboxes) {
    ...
    for (const [key, box] of Object.entries(styleboxes)) {
        if (box && typeof box === "object") {
            ...
            if (boxData.content_margin_left)
                content += `content_margin_left = ${boxData.content_margin_left}\n`;
            if (boxData.content_margin_right)
                content += `content_margin_right = ${boxData.content_margin_right}\n`;
            if (boxData.content_margin_top)
                content += `content_margin_top = ${boxData.content_margin_top}\n`;
            if (boxData.content_margin_bottom)
                content += `content_margin_bottom = ${boxData.content_margin_bottom}\n`;
            content += `\n`;
...
function generateStyleBoxFlat(bgColor, borderColor, borderWidth, cornerRadius, contentMargins, shadowColor, shadowSize) {
...
//# sourceMappingURL=ui-theme-tools.js.map
...
const LOOP_MODE_FROM_ID = {
    0: "none",
    1: "linear",
    2: "pingpong",
};
const INTERPOLATION_TO_ID = {
    nearest: 0,
    linear: 1,
    cubic: 2,
};
const INTERPOLATION_FROM_ID = {
    0: "nearest",
    1: "linear",
    2: "cubic",
};
const UPDATE_MODE_TO_ID = {
    continuous: 0,
    discrete: 1,
    capture: 2,
};
const UPDATE_MODE_FROM_ID = {
    0: "continuous",
    1: "discrete",
    2: "capture",
};
...
export function registerAnimationTools(tools, state) {
    tools.set("godot_animation_create_clip", {
        description: "Create an Animation resource (.tres/.anim) with value tracks and keyframes.",
        ...
        handler: async (args) => {
            const { path: clipPath, length, loopMode, step, tracks } = args;
    ...
    tools.set("godot_animation_read_clip", {
        description: "Read an Animation resource (.tres/.anim) and return structured tracks/keyframes.",
        inputSchema: z.object({
            path: z.string().describe("Path to animation resource"),
        }),
        handler: async (args) => {
            const { path: clipPath } = args;
            const fullPath = resolveProjectPath(clipPath, state.projectPath);
    ...
    tools.set("godot_animation_add_keyframe", {
        description: "Add a keyframe to an existing animation track, sorted by keyframe time.",
        inputSchema: z.object({
            path: z.string().describe("Path to animation resource"),
        ...
        handler: async (args) => {
            const { path: clipPath, trackIndex, time, value, transition } = args;
            const fullPath = resolveProjectPath(clipPath, state.projectPath);
            const content = await fs.readFile(fullPath, "utf-8");
            const clip = parseAnimationClipContent(content);
    ...
    tools.set("godot_animation_remove_keyframe", {
        description: "Remove a keyframe from an existing animation track by index or time.",
        inputSchema: z.object({
            path: z.string().describe("Path to animation resource"),
            trackIndex: z.number().int().min(0).describe("Track index"),
        ...
        handler: async (args) => {
            const { path: clipPath, trackIndex, keyframeIndex, time } = args;
    ...
    tools.set("godot_animation_list_clips", {
        description: "List Animation resources (.tres/.anim) in the project, including basic clip metadata.",
        inputSchema: z.object({
            directory: z
                .string()
                .optional()
    ...
    tools.set("godot_animation_setup_scene", {
        description: "Set up AnimationPlayer and optional AnimationTree nodes in a scene file.",
        inputSchema: z.object({
        ...
        handler: async (args) => {
            const { scenePath, parentPath, playerName, createTree, treeName, activeTree, } = args;
    ...
    tools.set("godot_animation_build_state_machine_plan", {
        description: "Build a state-machine configuration plan and optional script for AnimationTree runtime setup.",
        inputSchema: z.object({
        ...
        handler: async (args) => {
            const { states, transitions, entryState, outputPath } = args;
    ...
    tools.set("godot_animation_build_blend_space_plan", {
        description: "Build a blend-space configuration plan and optional script for AnimationTree setup.",
        inputSchema: z.object({
            blendMode: z
                .enum(["1d", "2d"])
                .default("2d")
        ...
        handler: async (args) => {
            const { blendMode, parameter, secondaryParameter, points, outputPath, } = args;
            const plan = buildBlendSpacePlan(blendMode, parameter, secondaryParameter, points);
            if (outputPath) {
                const fullPath = resolveProjectPath(outputPath, state.projectPath);
                await fs.mkdir(path.dirname(fullPath), { recursive: true });
                await fs.writeFile(fullPath, JSON.stringify(plan, null, 2), "utf-8");
...
export function buildStateMachinePlan(states, transitions, requestedEntryState) {
    const knownStates = new Set(states.map((state) => state.name));
    const missingTransitions = transitions
        .filter((transition) => !knownStates.has(transition.from) || !knownStates.has(transition.to))
    ...
    if (missingTransitions.length > 0) {
        throw new Error(`Transitions reference unknown states: ${missingTransitions.join(", ")}`);
    }
    const parameters = Array.from(new Set(transitions
...
export function buildBlendSpacePlan(blendMode, parameter, secondaryParameter, points) {
    ...
    return {
        ...
        points,
        requiredClips,
        generatedScript: generateBlendSpaceScript(blendMode, parameter, secondaryParameter, points),
    };
}
function generateStateMachineScript(entryState, states, transitions) {
    const lines = [];
    ...
    for (const state of states) {
        ...
        if (state.position && state.position.length === 2) {
            lines.push(`root.set_node_position(&"${safeState}", Vector2(${formatNumber(state.position[0])}, ${formatNumber(state.position[1])}))`);
        }
        lines.push("");
    }
    ...
    transitions.forEach((transition, index) => {
        const transitionVar = toScriptIdentifier(`transition_${transition.from}_${transition.to}_${index}`);
        ...
        lines.push(`${transitionVar}.xfade_time = ${formatNumber(transition.xfadeTime ?? 0.15)}`);
        if (transition.condition) {
            lines.push(`# Optional auto-advance condition: ${transitionVar}.advance_condition = &"${escapeGdString(transition.condition)}"`);
        }
        lines.push(`root.add_transition(&"${safeFrom}", &"${safeTo}", ${transitionVar})`);
        lines.push("");
    });
    lines.push("var playback := tree.get(\"parameters/playback\")");
    lines.push(`if playback is AnimationNodeStateMachinePlayback:\n\tplayback.travel(&"${escapeGdString(entryState)}")`);
    return lines.join("\n");
}
function generateBlendSpaceScript(blendMode, parameter, secondaryParameter, points) {
    const lines = [];
    lines.push("# Generated by godot_animation_build_blend_space_plan");
    lines.push("var tree := $AnimationTree");
    lines.push(`var blend := ${blendMode === "1d" ? "AnimationNodeBlendSpace1D" : "AnimationNodeBlendSpace2D"}.new()`);
    ...
    for (const point of points) {
        ...
        if (blendMode === "1d") {
            lines.push(`blend.add_blend_point(${pointVar}, ${formatNumber(point.position[0])})`);
        }
        else {
            lines.push(`blend.add_blend_point(${pointVar}, Vector2(${formatNumber(point.position[0])}, ${formatNumber(point.position[1])}))`);
        }
        lines.push("");
    }
    lines.push(`# Drive blend with parameters '${parameter}'${blendMode === "2d" ? ` and '${secondaryParameter}'` : ""}`);
    if (blendMode === "1d") {
        lines.push(`tree.set("parameters/${escapeGdString(parameter)}", 0.0)`);
    }
    else {
        lines.push(`tree.set("parameters/${escapeGdString(parameter)}", Vector2(0.0, 0.0))`);
    }
    return lines.join("\n");
}
function toScriptIdentifier(raw) {
...
export function serializeAnimationClipContent(clip) {
    const lines = [];
...
export function parseAnimationClipContent(content) {
    const lines = content.split("\n");
    let length = 1;
    let loopMode = "none";
    let step = 1 / 30;
    const partialTracks = new Map();
    for (const rawLine of lines) {
        ...
        if (typeMatch) {
            ...
            if (!partialTracks.has(index))
                partialTracks.set(index, {});
            continue;
        }
...
function parseTrackKeys(keysLiteral) {
    const timesMatch = keysLiteral.match(/"times"\s*:\s*PackedFloat32Array\(([^)]*)\)/);
    const transitionsMatch = keysLiteral.match(/"transitions"\s*:\s*PackedFloat32Array\(([^)]*)\)/);
    ...
    const updateMode = UPDATE_MODE_FROM_ID[parseIntSafe(updateMatch?.[1], 0)] || "continuous";
...
function parsePackedFloatArray(raw) {
    if (!raw)
        return [];
...
function serializeTrackKeys(track) {
    ...
    return `{"times": PackedFloat32Array(${times.join(", ")}), "transitions": PackedFloat32Array(${transitions.join(", ")}), "update": ${UPDATE_MODE_TO_ID[track.updateMode]}, "values": [${values.join(", ")}]} `;
}
function serializeVariant(value) {
    if (value === null)
        return "null";
...
function summarizeClip(clip) {
    const keyframeCount = clip.tracks.reduce((acc, track) => acc + track.keyframes.length, 0);
    return {
        length: clip.length,

...
export function registerUILayoutTools(
  ...
  tools.set("godot_ui_list_anchors", {
    description:
      "List all available anchor presets for positioning UI elements. Use these presets for responsive layouts.",
  ...
  tools.set("godot_ui_list_containers", {
    description:
      "List all available container types for organizing UI elements with their use cases.",
    inputSchema: z.object({}),
    handler: async () => {
      return {
        containers: [
  ...
  tools.set("godot_ui_create_layout", {
    description:
      "Create a responsive layout scene with specified structure. Good for creating screen layouts that adapt to different resolutions.",
    inputSchema: z.object({
    ...
    handler: async (args) => {
      const { path: scenePath, layout_type, theme_path, options } = args as {
        path: string;
        layout_type: string;
        theme_path?: string;
        options?: {
  ...
  tools.set("godot_ui_get_anchor_config", {
    description:
      "Get the Godot scene file properties for a specific anchor configuration. Useful for understanding how to configure anchors manually.",
    inputSchema: z.object({
      preset: z.enum([
    ...
    handler: async (args) => {
      const { preset, margins } = args as {
        preset: keyof typeof ANCHOR_PRESETS;
      ...
      if (config.anchor_left !== 0) tscnProperties += `anchor_left = ${config.anchor_left}\n`;
      if (config.anchor_top !== 0) tscnProperties += `anchor_top = ${config.anchor_top}\n`;
      if (config.anchor_right !== 0) tscnProperties += `anchor_right = ${config.anchor_right}\n`;
  ...
  tools.set("godot_ui_create_container", {
    description:
      "Create a container scene with specified configuration.",
    inputSchema: z.object({
      path: z.string().describe("Output path for the scene file"),
    ...
    handler: async (args) => {
      const { path: scenePath, container_type, anchor_preset, properties, theme_path } = args as {
        path: string;
        container_type: string;
        anchor_preset?: string;
        properties?: { separation?: number; columns?: number; margins?: number };
        theme_path?: string;
      };

      const content = generateContainerScene(
        container_type,
...
function getAnchorDescription(name: string): string {
...
function generateAnchorGDScript(
  preset: string,
  margins?: { left?: number; top?: number; right?: number; bottom?: number }
): string {
  const config = ANCHOR_PRESETS[preset as keyof typeof ANCHOR_PRESETS];
  if (!config) return "";
  ...
  if (margins) {
    if (margins.left) code += `control.offset_left = ${margins.left}\n`;
    if (margins.top) code += `control.offset_top = ${margins.top}\n`;
    if (margins.right) code += `control.offset_right = ${margins.right}\n`;
    if (margins.bottom) code += `control.offset_bottom = ${margins.bottom}\n`;
  }
...
function generateContainerScene(
  ...
  const anchor = ANCHOR_PRESETS[anchorPreset as keyof typeof ANCHOR_PRESETS] || ANCHOR_PRESETS.full_rect;

  content += `[node name="${containerType.replace("Container", "")}" type="${containerType}"]\n`;
  content += `layout_mode = 3\n`;
  content += `anchors_preset = ${anchor.preset}\n`;

  if (anchor.anchor_left !== 0) content += `anchor_left = ${anchor.anchor_left}\n`;
  if (anchor.anchor_top !== 0) content += `anchor_top = ${anchor.anchor_top}\n`;
  if (anchor.anchor_right !== 0) content += `anchor_right = ${anchor.anchor_right}\n`;
  if (anchor.anchor_bottom !== 0) content += `anchor_bottom = ${anchor.anchor_bottom}\n`;

...
export function registerInputTools(
  ...
  tools.set("godot_input_list_actions", {
    description:
      "List InputMap actions from project.godot with deadzones and binding counts.",
    ...
    handler: async (args) => {
      const { includeEvents } = args as { includeEvents: boolean };
  ...
  tools.set("godot_input_get_action", {
    description:
      "Get a single InputMap action and its bindings from project.godot.",
    inputSchema: z.object({
  ...
  tools.set("godot_input_set_action", {
    description:
      "Create or update an InputMap action in project.godot (replace or append bindings).",
    inputSchema: z.object({
      action: z.string().min(1).describe("Input action name"),
      deadzone: z
        .number()
    ...
    handler: async (args) => {
      const { action, deadzone, events, replaceEvents } = args as {
        action: string;
        deadzone?: number;
        events?: Array<z.infer<typeof InputEventSchema>>;
      ...
      if (!existing && (!normalizedEvents || normalizedEvents.length === 0)) {
        throw new Error(
          "New actions require at least one event binding. Provide `events` with one or more entries."
        );
      }

      const nextAction: InputAction = {
        name: action,
      ...
      const nextContent = updateInputSection(
        file.content,
        Array.from(actionMap.values()).sort((a, b) => a.name.localeCompare(b.name))
      );
  ...
  tools.set("godot_input_remove_action", {
    ...
    handler: async (args) => {
      const { action, errorIfMissing } = args as {
        action: string;
  ...
  tools.set("godot_input_apply_preset", {
    description:
      "Apply a built-in InputMap preset to project.godot (merge or replace existing actions).",
    inputSchema: z.object({
      preset: z
    ...
    handler: async (args) => {
      const { preset, merge } = args as {
        preset: keyof typeof INPUT_PRESETS;
        merge: boolean;
      };
      ...
      let nextActions: InputAction[];
      if (merge) {
        const actionMap = new Map(parsed.actions.map((entry) => [entry.name, entry]));
        for (const action of presetActions) {
          actionMap.set(action.name, action);
        }
        nextActions = Array.from(actionMap.values()).sort((a, b) =>
...
async function readProjectFile(projectPath: string | null): Promise<{
  fullPath: string;
  projectPath: string;
  content: string;
}> {
  ...
  try {
    ...
    throw new Error(
      "project.godot not found. Initialize a project first with `godot_init_project` or point --project to a valid Godot project."
    );
  }
...
export function parseInputMapContent(content: string): {
...
export function updateInputSection(content: string, actions: InputAction[]): string {
  const extracted = extractInputSection(content);
...
function extractInputSection(content: string): {
  exists: boolean;
  before: string;
  body: string;
  after: string;
...
function parseInputActions(body: string): InputAction[] {
...
function parseInputActionLiteral(name: string, literal: string): InputAction {
  const deadzoneMatch = literal.match(
    /"deadzone"\s*:\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)/
  );
...
function extractNamedArrayLiteral(source: string, name: string): string | null {
  const key = `"${name}"`;
...
function parseInputEvent(rawEvent: string): InputEvent {
...
function parseObjectFields(raw: string): Record<string, string> {
...
function normalizeEventForWrite(event: z.infer<typeof InputEventSchema>): InputEvent {
  if (event.type === "raw") {
...
function serializeInputSection(actions: InputAction[]): string {
...
function countBraces(input: string): number {
  let depth = 0;
  let inString = false;
...
const INPUT_PRESETS: Record<string, InputAction[]> = {
  "2d_platformer": [
...
function keyAction(name: string, keys: number[], deadzone = 0.5): InputAction {
  return {
    name,
    deadzone,

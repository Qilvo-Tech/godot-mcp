...
interface ShaderMetadata {
  ...
  varyings: Array<{ name: string; type: string }>;
  functions: string[];
}
...
export function registerShaderTools(
  ...
  tools.set("godot_read_shader", {
    description:
      "Read a Godot shader file (.gdshader) and return its content along with parsed metadata (type, uniforms, functions).",
  ...
  tools.set("godot_write_shader", {
    description:
      "Create or update a Godot shader file (.gdshader). Writes the provided shader code to the file.",
    inputSchema: z.object({
      path: z.string().describe("Path where the shader should be written"),
      content: z.string().describe("The shader code to write"),
    }),
    handler: async (args) => {
      const { path: shaderPath, content } = args as {
        path: string;
        content: string;
      };
      const fullPath = resolveProjectPath(shaderPath, state.projectPath);
  ...
  tools.set("godot_generate_shader", {
    description:
      "Generate a Godot shader based on a description and shader type. Creates properly structured .gdshader code.",
    inputSchema: z.object({
      description: z
        .string()
    ...
    handler: async (args) => {
      const { description, shaderType, effect } = args as {
        description: string;
        shaderType: string;
        effect?: string;
      };
  ...
  tools.set("godot_list_shaders", {
    ...
    handler: async (args) => {
      ...
      const shaderInfos = await Promise.all(
        shaders.map(async (shaderPath) => {
          try {
...
// Shader parser
function parseShader(content: string): ShaderMetadata {
  ...
  for (const line of lines) {
    ...
    const uniformMatch = trimmed.match(
      /^(?:uniform|instance\s+uniform)\s+(\w+)\s+(\w+)(?:\s*:\s*([^=]+))?(?:\s*=\s*(.+))?;/
    );
    if (uniformMatch) {
...
function getEffectTemplate(effect: string, shaderType: string): string {

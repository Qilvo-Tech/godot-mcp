...
interface ScriptMetadata {
  ...
  extends?: string;
  signals: string[];
  exports: Array<{ name: string; type: string; default?: string }>;
  onready: Array<{ name: string; type: string; path: string }>;
  functions: Array<{ name: string; args: string[]; returnType?: string; isOverride: boolean }>;
...
export function registerScriptTools(
  ...
  tools.set("godot_read_script", {
    description:
      "Read a GDScript file (.gd) and return its content along with parsed metadata (class name, signals, exports, functions, etc.).",
    inputSchema: z.object({
      path: z.string().describe("Path to the GDScript file"),
    }),
    handler: async (args) => {
      const { path: scriptPath } = args as { path: string };
  ...
  tools.set("godot_write_script", {
    ...
    handler: async (args) => {
      const { path: scriptPath, content } = args as {
        path: string;
        content: string;
      };
      const fullPath = resolveProjectPath(scriptPath, state.projectPath);
  ...
  tools.set("godot_analyze_script", {
    description:
      "Analyze a GDScript file in detail, extracting all classes, functions, signals, exports, and their relationships.",
    inputSchema: z.object({
  ...
  tools.set("godot_generate_script", {
    description:
      "Generate a GDScript file based on a description. Creates properly structured Godot 4.x code with type hints, signals, and best practices.",
    inputSchema: z.object({
      description: z
        .string()
        .describe(
          "Description of what the script should do (e.g., 'A player controller with WASD movement and jumping')"
        ),
      extends: z
        .string()
        .default("Node")
      ...
      features: z
        ...
        .optional()
        .describe(
          "Specific features to include (e.g., ['signals', 'exports', 'ready', 'process'])"
        ),
    }),
    handler: async (args) => {
      const { description, extends: baseClass, className, features } = args as {
        description: string;
        extends: string;
        className?: string;
        features?: string[];
  ...
  tools.set("godot_validate_script", {
    description:
      "Validate a GDScript file for common syntax issues and best practices.",
    inputSchema: z.object({
    ...
    handler: async (args) => {
      const { path: scriptPath, content: directContent } = args as {
        path?: string;
        content?: string;
      };
...
// GDScript parser
function parseGDScript(content: string): ScriptMetadata {
  ...
  for (const line of lines) {
    ...
    const exportMatch = trimmed.match(
      /^@export(?:_\w+)?\s+var\s+(\w+)\s*:\s*(\w+)(?:\s*=\s*(.+))?/
    );
    if (exportMatch) {
...
function validateScript(
  content: string
): Array<{ severity: string; message: string; line?: number }> {
...
function generateScript(
  ...
  const hasMovement =
    description.toLowerCase().includes("movement") ||

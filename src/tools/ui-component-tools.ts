...
export function registerUIComponentTools(
  ...
  tools.set("godot_ui_create_menu", {
    description:
      "Create a menu scene with a title, buttons, and optional background. Perfect for main menus, pause menus, and settings screens.",
  ...
  tools.set("godot_ui_create_hud", {
    description:
      "Create a game HUD scene with common elements like score, health, timer, and minimap placeholder.",
    inputSchema: z.object({
    ...
    handler: async (args) => {
      const { path: scenePath, elements, layout, theme_path } = args as {
        path: string;
        elements: string[];
        layout?: string;
        theme_path?: string;
  ...
  tools.set("godot_ui_create_dialog", {
    description:
      "Create a dialog/popup scene for confirmations, alerts, or custom content.",
    inputSchema: z.object({
      path: z.string().describe("Output path for the scene file"),
    ...
    handler: async (args) => {
      const { path: scenePath, title, message, buttons, dialog_type, theme_path, width } = args as {
        path: string;
        title: string;
        message?: string;
        buttons?: Array<{ text: string; name?: string; signal?: string }>;
  ...
  tools.set("godot_ui_create_panel", {
    description:
      "Create a styled panel container that can hold other UI elements.",
    inputSchema: z.object({
      path: z.string().describe("Output path for the scene file"),
    ...
    handler: async (args) => {
      const { path: scenePath, title, size, position, theme_path, content_type } = args as {
        path: string;
        title?: string;
      ...
      const content = generatePanelScene(
        title,
        size,
        position || "center",
        theme_path,
...
// Generate HUD scene
function generateHUDScene(elements: string[], layout: string, themePath?: string, scriptPath?: string): string {
  let extResourceCount = 0;
  let extResources = "";
...
function generateHUDElement(element: string, parent: string): string {
  let content = "";
...
function elementToNodeName(element: string): string {
  return element
    .split('_')
...
// Generate HUD script
function generateHUDScript(elements: string[], layout: string = "top_bar"): string {
  let script = `extends CanvasLayer\n\n`;

  // Determine container paths based on layout
  const getContainerPath = (element: string): string => {
    if (layout === "top_bar") {
      return "TopBar";
    } else if (layout === "corners") {
      if (element === "minimap") return "BottomRight";
      if (element === "timer" || element === "score") return "TopRight";
      if (element === "lives" || element === "level") return "BottomLeft";
      return "TopLeft";
    } else {
      return "CustomLayout";
    }
...
function generatePanelScene(
  title: string | undefined,
  size: { width: number; height: number } | undefined,
  position: string,
  themePath?: string,
  contentType: string = "vbox"
): string {

...
export function registerSceneTools(
  ...
  tools.set("godot_read_scene", {
    description:
      "Read and parse a Godot scene file (.tscn). Returns the scene structure as JSON including nodes, resources, and connections.",
    inputSchema: z.object({
      path: z
        .string()
        .describe(
          "Path to the scene file, either absolute or relative to project (e.g., 'res://scenes/player.tscn' or 'scenes/player.tscn')"
        ),
    }),
    handler: async (args) => {
  ...
  tools.set("godot_write_scene", {
    description:
      "Create or update a Godot scene file (.tscn) from structured JSON data. Use this to create new scenes or overwrite existing ones.",
    inputSchema: z.object({
      path: z
        .string()
    ...
    handler: async (args) => {
      const { path: scenePath, scene } = args as {
        path: string;
      ...
      const completeScene: ParsedScene = {
        header: {
          ...
          loadSteps:
            (scene.externalResources?.length || 0) +
  ...
  tools.set("godot_add_node", {
    description:
      "Add a new node to an existing Godot scene file. The node will be added to the scene tree at the specified parent path.",
    inputSchema: z.object({
  ...
  tools.set("godot_remove_node", {
    description:
      "Remove a node and all its children from a Godot scene file.",
    inputSchema: z.object({
      scenePath: z.string().describe("Path to the scene file to modify"),
      nodePath: z
        .string()
        .describe(
          "Path to the node to remove (e.g., 'Player' for root child, 'Player/Sprite' for nested)"
        ),
    }),
    handler: async (args) => {
      const { scenePath, nodePath } = args as {
        scenePath: string;
        nodePath: string;
      };
      const fullPath = resolveProjectPath(scenePath, state.projectPath);
  ...
  tools.set("godot_modify_node", {
    description:
      "Modify properties of an existing node in a Godot scene file.",
    inputSchema: z.object({
      scenePath: z.string().describe("Path to the scene file to modify"),
    ...
    handler: async (args) => {
      const { scenePath, nodePath, updates } = args as {
        scenePath: string;
        nodePath: string;
  ...
  tools.set("godot_list_scene_nodes", {
    description:
      "List all nodes in a Godot scene file with their types and hierarchy.",
    inputSchema: z.object({
  ...
  tools.set("godot_validate_scene", {
    description:
      "Validate a Godot scene file for common issues like missing resources, invalid node types, or broken references.",
    inputSchema: z.object({
